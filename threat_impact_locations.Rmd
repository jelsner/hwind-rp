...
Create a model that can be used to outline areas with dates of hurricane threats and impacts. Model output can be collated with health outcome data at the level of individuals within the areas for those specific dates.
...
Keep only hurricanes between 1981 and 2022
```{r}
...
```{r}
health.df <- read.csv(file = "data/all_deaths_hurricane.csv")
```
...
## Determine the dates in `health.sf` that correspond with the dates in `expanded_Swaths.sf`

```{r}
X <- health.sf$Date %in% expanded_Swaths.sf$Dates
sum(X)
```

68424 death dates correspond to hurricane threat/impact dates in Florida. But not all occur within the threat/impact zone

## Determine the deaths occurring within the threat/impact zones

Filter out deaths not occurring on hurricane threat/impact days in Florida

```{r}
health2.sf <- health.sf[X,]
```

Filter out deaths not occurring within the threat/impact areas
...
health2.sf <- health2.sf[X,]
```

47386 death dates corresponding to hurricane threat/impact dates and occurring within the union of all threat/impact areas

For each death assign it to the appropriate date and hurricane
```{r}
indicator <- rep(-3:3, times = 24)
expanded_Swaths.sf$indicator <- indicator

TI <- NULL
for(i in 1:nrow(health2.sf)){
TI <- c(TI, expanded_Swaths.sf$indicator[expanded_Swaths.sf$Dates == health2.sf$Date[i]])
}
health2.sf$TI <- TI
```
