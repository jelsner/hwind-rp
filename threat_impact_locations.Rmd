...
Tracks2.sf <- Tracks.sf |>
  dplyr::filter(year >= 1981 & year <= 2022) |>
  dplyr::filter(USA_WIND >= 34) |> # changed from 64 for hurricanes
...
6097671 death dates correspond to dates outside TIC dates in Florida.
...
47836/168 = 282 deaths per day
4184405/15172 = 276 deaths per day

Approximately 6 extra deaths per day due to threat/impact/cleanup. What is the uncertainty on this estimate? One way to answer this is to choose random dates. `Date0` in `Swaths.sf` 
...
3. Obtain the polygon for your area
...
5. Filter census data by the polygon
...
47,386 deaths occurring in these regions occurring over 168 days (7 days x 24 hurricanes)
